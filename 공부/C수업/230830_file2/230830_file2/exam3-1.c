...
int menu();
void write(CUST data[]);		//수동입력함수
void ran_write(CUST data[]);	//랜덤입력함수
void read();					//읽기
void del();
int main()
{
...
void write(CUST data[])
{
	FILE* fp = fopen("person_car.bin", "ab");
	if (fp == NULL) {
		printf("파일 쓰기 실패!!\n");
		return -1;
	}
...
void read()
{
	FILE* fp = fopen("person_car.bin", "rb");
	if (fp == NULL) {
		printf("파일 읽기 실패!!!\n");
		return -1;
	}
...
void del()
{
	FILE* fp = fopen("person_car.bin", "wb");
	fclose(fp);
	printf("파일 삭제 완료!!\n");
}